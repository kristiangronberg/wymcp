defmodule Wymcp.Tool.Schema do
  ...
  whose description carries the action summaries (via `action_summaries/1`),
  one per line, and `data` is a bare object. The newline separator stays
  unambiguous because `Wymcp.Tool.validate_actions!/1` — run at boot and at
  runtime registration — rejects a newline in an action's name or its
  description, the two halves a summary joins. `Wymcp.Tool`'s generated
  `input_schema/0` is the only caller.
