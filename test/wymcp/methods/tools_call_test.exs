defmodule Wymcp.Methods.ToolsCallTest do
  ...
  If a tool's `run/2` raises, the exception is caught and reported as
  a successful JSON-RPC response with `isError: true` and a JSON-encoded
  diagnostic body, per the MCP 2025-11-25 spec.
  ...
  `run_action/3` produces a successful JSON-RPC response with an error result.
  """
